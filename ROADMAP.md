...
| Фаза | Разделы | Зачем |
|------|---------|--------|
| **A. Основа** | ~~4~~ → 5 → 6 | ~~Тесты (базовые)~~ ✓; QG; контракт API |
| **B. Ядро продукта** | ~~1~~ → 3 | ~~Захват/ROI~~ ✓; зафиксировать политику снимков |
...
## 1. Исправить ROI на снимке (режим захвата) — ✅ сделано

**Проблема:** при захвате ROI на экране не совпадал с выборкой; цвет на снимке пересчитывался один раз; на JPEG оставалась вшитая жёлтая рамка ROI.

**Сделано (2026-05):**

- [x] Захват без overlay: `GET /api/snapshot/{role}?overlay=false`, сырой кадр отдельно от превью
- [x] Пересчёт цвета при каждом сдвиге ROI: не отзывать `blob:` URL при обновлении только `color`
- [x] Letterboxing: `pointerToFrame` / `imageLayout.ts`, ref на `<img>` в overlay
- [ ] Unit-тесты `sampleImageColor` и маппинга — вынести в **п. 4**

**Критерий:** ✓ сдвиг ROI на снимке меняет цвет; на фото нет «призрачной» рамки с бэкенда.

**Блокировало:** п. 2.3, п. 10 — снято для 2.3; для Telegram по-прежнему нужен п. 2–3.
...
*Обновлено: 2026-05-23 (п. 1 — выполнен)*
